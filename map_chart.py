...
def prepare_region_data(filepath='./assets/data/data_fusionnee.csv'):
    '''
...
def draw_geo_map(df_counts):
    '''
    Trace une carte interactive avec des bulles pour les régions.

    Args:
        df_counts (pd.DataFrame): données avec colonnes region, nb_accidents, latitude, longitude

    Returns:
        plotly.graph_objects.Figure: la figure Plotly
    '''
    print(df_counts[['region', 'nb_accidents', 'latitude', 'longitude']].head())
    fig = px.scatter_geo(
        df_counts,
        lat='latitude',
        lon='longitude',
        ...
        title='Road Accidents by Region in Quebec'
    )
